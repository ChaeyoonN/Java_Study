package basic.array;

public class Array2DQuiz {

	public static void main(String[] args) {
		int[][] score = {
                {79, 80, 99}, //A학생
                {95, 85, 89}, //B학생
                {90, 65, 56}, //C학생
                {69, 78, 77}  //D학생
                //과목: 3과목
        };
        String[] stuName = {"A학생", "B학생", "C학생", "D학생"};
        String[] subName = {"국어", "영어", "수학"};
        
        /*
        1. 각 학생의 평균을 소수점 첫째 자리까지 출력해 보세요.
        2. 각 과목의 평균을 출력해 보세요.
        3. 반 평균을 출력해 보세요. (모든 학생들의 평균을 더해서 학생수로 나누기)
        */
        
        //1.
        double totalAvg = 0.0;//학생들의 평균점수의 총합
        int idx=0; //stuName 배열에서 학생의 이름을 꺼내올 용도로 사용할 변수.
        for(int[] arr:score) {
        	int sum=0;
        	
        	for(int i=0; i<arr.length; i++) {
        		
//        		System.out.println(arr[i]+" ");
        		sum += arr[i];
        	}
        	double avg = (double) sum/arr.length;
        	totalAvg += avg;
        	System.out.printf("%s의 평균: %.1f\n",stuName[idx],avg);
        	idx++;
        }
        
        System.out.println("----------------------");
        //2.
//        int t = score[0][0]+score[1][0]+score[2][0]+score[3][0];
//        int t2 = score[0][1]+score[1][1]+score[2][1]+score[3][1];
//        int a = t/subName.length;
        
        for(int subject=0; subject<subName.length; subject++) {
        	int sum=0;
        	
        	for(int stu=0; stu<stuName.length; stu++) {
//        		System.out.println(score[stu][subject]+" ");
        		sum += score[stu][subject];
        		
        	}
        	double avg = (double) sum/stuName.length;
        	System.out.printf("%s의 평균점수: %.1f점\n",subName[subject],avg);
        }
        
        System.out.println("----------------------");
        //3.
        double classAvg = totalAvg/stuName.length;
        System.out.printf("우리 반 평균: %.1f점\n",classAvg);
        
        
        
        

	}
}